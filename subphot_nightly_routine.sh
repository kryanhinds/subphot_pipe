...
if [ $(date +%H%M) -gt 0000 ] && [ $(date +%H%M) -lt 0800 ]; then
    ...
    python3 /users/arikhind/subphot_pipe/subphot_subtract.py -f Quicklook/$(date -d "yesterday" +%Y%m%d) -swarp -psfex -o by_obs_date -new -cut -up -pid $(date -d "yesterday" +%Y%m%d%H%M)  2>&1| tee /mnt/data1/users/arikhind/phot_data/nightly_routine_logs/$(date -d "yesterday" +%Y%m%d)/nightly_routine_log_$(date +%Y%m%d%H%M)_bash.log
