    ...
    try:
        ...
        if _has_cd and not (_has_old_pc or _has_modern_pc or _has_cdelt):
            if logger is not None:
                logger.info(info_g+f' [V2] Reference WCS already in CD-matrix form '
...
class subtracted_phot(subphot_data):
                ...
                if any(t in header_kw.keys() for t in [self.telescope,self.telescope.upper()]):
                    ...
                    if self.telescope=='Liverpool Telescope':
                        ...
                        if self.redo_astrometry==True:
                            ...
                            # Pass SN pixel position so the trim never cuts past the
                            # target (TARGXPX/TARGYPX are written by the SEDM pipeline,
                            # 0-indexed column/row of the target object).
    ...
    def swarp_ref_align(self,image_size=image_size):
        ...
        self.sp_logger.info(info_g+f' [DEBUG] survey_name={survey_name}, use_legacy_survey={use_legacy_survey}')
        ...
        if self.auto_ref=='auto':
            ...
            if use_legacy_survey:
                if self.sci_filt in ['g', 'r', 'z']:
                    self.sp_logger.info(info_g+f' Downloading Legacy Survey reference image for {self.sci_filt}-band')
                    ...
                    if ref_path is None:
                        self.sp_logger.warning(warn_r+f' Legacy Survey download failed, falling back to SDSS')
                        self.ref_img_name = self.make_sdss_ref(sdss_filt=self.sci_filt)
                        if self.ref_img_name is None:
                            self.sp_logger.warning(warn_r+f' SDSS reference also unavailable, proceeding without reference image')
                            self.ref_path = None
                        else:
                        ...
                        try:
                            ...
                            if ref_shape[0] != expected_size or ref_shape[1] != expected_size:
                                self.sp_logger.warning(warn_y+f' Reference image size {ref_shape} does not match expected {expected_size}x{expected_size} for {self.telescope}')
                                self.sp_logger.info(info_g+f' Deleting cached reference and re-downloading...')
                                ...
                                if ref_path is not None:
                                    self.ref_img_name = ref_path
                                    self.ref_path = ref_path
                                    self.sp_logger.info(info_g+f' Downloaded correct reference image: '+self.ref_img_name)
                            else:
                                self.sp_logger.info(info_g+f' Reference image size verified: {ref_shape[0]}x{ref_shape[1]}')
                        except Exception as e:
                            self.sp_logger.warning(warn_y+f' Could not verify reference image size: {e}')

                        self.sp_logger.info(info_g+f' Using Legacy Survey reference image: '+self.ref_img_name)
                        panstamps_status=100
                else:
                    self.sp_logger.warning(warn_r+f' Legacy Survey not available for {self.sci_filt}-band, using SDSS')
                    self.ref_img_name = self.make_sdss_ref(sdss_filt=self.sci_filt)
                    if self.ref_img_name is None:
                        self.sp_logger.warning(warn_r+f' SDSS reference unavailable, proceeding without reference image')
                        self.ref_path = None
                    else:
                        self.ref_path = self.ref_img_name
                    panstamps_status=100
        ...
        if self.telescope in SEDM:
            ...
            try:
                ...
                self.sp_logger.warning(warn_y + f' [V2] reproject_interp raised {type(_rep_e).__name__}: '
                               f'{str(_rep_e)[:120]}\n'
            ...
            if _reproject_failed or not footprint_bool.any():
                ...
                try:
                    ...
                    self.sp_logger.warning(warn_r + f' [V2] reproject_interp also failed on the '
                    ...
                    if os.path.exists(_cat_path):
                        ...
                        if _ra_col and _dec_col:
                            _align_cat_sky = SkyCoord(
                                ...
                                frame='fk5')
                            self.sp_logger.info(info_g + f' [V2] User refcat alignment catalogue: '
                                           f'{len(_align_cat_sky)} stars from '
                                           f'{os.path.basename(_cat_path)}')
                        else:
                            self.sp_logger.warning(warn_y + f' [V2] User refcat lacks ra/dec columns; '
                                           'falling through to default fine-reg')
                    else:
                        self.sp_logger.warning(warn_y + f' [V2] User refcat path not found: {_cat_path}')
                except Exception as _uc_e:
                    self.sp_logger.warning(warn_y + f' [V2] User refcat load failed: {_uc_e}')
            ...
            if _freg_method == 'none':
                try:
                    dx, dy = _matched_filter_shift(
                        ...
                        logger=self.sp_logger)
                    _freg_method = 'pcc'
                    self.sp_logger.info(info_g+f' [V2] FFT matched-filter fine reg: '
                                        f'dx={dx:.3f} px, dy={dy:.3f} px')
                except Exception as _pcc_e:
                    self.sp_logger.warning(warn_y+f' [V2] FFT matched-filter fine reg failed: {_pcc_e}')
            ...
            if _freg_method != 'none' and (abs(dx) > 0.05 or abs(dy) > 0.05):
                if abs(dx) < 50 and abs(dy) < 50:
                    ...
                    # decisively — SEDM ACQ pointing WCS can genuinely be off by
                    # 1-2 arcmin (~150-300 px); an FFT locked onto noise won't
                    # survive this check.  Raw masked NCC, not high-passed:
                    # high-passing kills the correlation between a ~3" seeing
                    # SEDM frame and a sharp survey reference even at perfect
                    # alignment (measured 0.03-0.06 high-passed vs 0.33-0.42 raw
                    # for correct shifts).
                    ...
                    if (np.isfinite(_ncc_after) and _ncc_after >= 0.10
                        ...
                        self.ref_resampled = scipy.ndimage.shift(
                            ...
                            order=3, mode='constant', cval=ref_median)
                        _shift_applied = True
                        self.sp_logger.info(info_g+f' [V2] [{_freg_method}] Large fine shift '
                              f'({dx:.3f}, {dy:.3f}) px ACCEPTED after NCC validation '
                              f'(NCC {_ncc_before:.3f} -> {_ncc_after:.3f})')
                    else:
                        self.sp_logger.warning(warn_y+f' [V2] Fine shift ({dx:.3f}, {dy:.3f}) px exceeds '
                              f'50-pixel safety limit and failed NCC validation '
                              f'(NCC {_ncc_before:.3f} -> {_ncc_after:.3f}) — skipping')

            # [V2] The measured shift IS the science-WCS pointing error: fold it
            # into CRPIX so downstream catalog-star pixel positions and the SN
            # photometry position (read from the aligned/convolved files written
            # from this header) land on the actual stars.
            if _shift_applied:
                ...
                self.sp_logger.info(info_g+f' [V2] Science WCS corrected: CRPIX += ({dx:.2f}, {dy:.2f}) px '
            ...
            if use_legacy_survey:
                ...
                self.sp_logger.info(info_g + f' [V2] Pre-scaled Legacy reference by '
            ...
            if use_legacy_survey:
                ...
                if _sat_core.any():
                    ...
                    self.sp_logger.info(info_g + f' [V2] Legacy Survey saturation mask: '
        ...
        while self.align_success==False:
            ...
            if (not try_aa_instead) and np.shape(fits.open(self.sci_ali_name)[0].data)!=np.shape(fits.open(self.ref_ali_name)[0].data) and ((self.image_size,self.image_size)!=np.shape(fits.open(self.sci_ali_name)[0].data) or (self.image_size,self.image_size)!=np.shape(fits.open(self.ref_ali_name)[0].data)):
                ...
                try:
                    ...
                    self.sp_logger.info(info_g+f' [V2] Aligning image sizes: science={sci_shape}, reference={ref_shape}')
                    ...
                    try:
                        if sci_shape == ref_shape:
                            ...
                            self.sp_logger.info(info_g+' Science and reference shapes match, no alignment needed')
                            ...
                            self.sp_logger.info(info_g+f' Science COMIN1,COMIN2: {sci_cominx} {sci_cominy} '
                            ...
                            if pad_y_start < pad_y_end and pad_x_start < pad_x_end:
                                ...
                                self.sp_logger.info(info_g+f' [V2] Successfully aligned images: placed {valid_shape} science data into {ref_shape} reference grid')
                            else:
                                # No overlap — shouldn't happen, but fall back to SWarp retry
                                self.sp_logger.warning(warn_y+f' [V2] Science and reference images do not overlap after offset correction')
                                self.align_success = False
                                continue

                        # Success — use aligned images
                        self.sp_logger.info(info_g+' [V2] Image alignment complete')
                    except Exception as e:
                        self.sp_logger.warning(warn_y+f' [V2] Image alignment failed: {e}')
                        self.sp_logger.warning(warn_y+f' Falling back to SWarp retry')
                        self.align_success = False
                        continue
                    ...
                    self.sp_logger.info(info_g+f' [V2] Valid pixel mask created: {self.valid_mask.sum()} / {self.valid_mask.size} pixels are real data ({100*self.valid_mask.mean():.1f}%)')
            ...
            try:
                ...
                if len(_ref_cat) >= 3 and _ref_cat[2].data is not None and len(_ref_cat[2].data) > 5:
                    ...
                    if int(_keep.sum()) >= 5:
                        ...
                        self.sp_logger.info(info_g + f' [V2] Ref star-locus filter: kept '
                                       f'{int(_keep.sum())}/{_n0} sources '
                                       f'(SNR≥20, elong≤1.3, FLUX_RADIUS within '
                                       f'25% of median)')
                    else:
                        self.sp_logger.warning(warn_y + f' [V2] Ref star-locus filter would leave '
                                       f'only {int(_keep.sum())} sources — keeping '
                                       f'all {_n0} for PSFEx to choose from')
                    _ref_cat.close()
            except Exception as _ref_filt_e:
                self.sp_logger.warning(warn_y + f' [V2] Ref catalog filter failed: {_ref_filt_e}')
    ...
    def gen_ref_cat(self):
        ...
        if _det_nz.size > 1000:
            ...
            if np.isfinite(_det_mad_std) and _det_mad_std > 0 and self.std > 2.5 * _det_mad_std:
                self.sp_logger.info(info_g+f' [V2] Detection std {self.std:.1f} > 2.5x MAD ({_det_mad_std:.1f}) — '
        ...
        if self.sources is None or len(self.sources) == 0:
            self.sp_logger.warning(warn_y + ' [V2] IRAFStarFinder found 0 sources above threshold '
        ...
        if _zm.any() and len(self.star_coords_pix) > 0:
            ...
            if _keep.sum() > 0 and (~_keep).sum() > 0:
                self.sp_logger.info(info_g+f' [V2] Dropped {(~_keep).sum()}/{len(_keep)} detections '
                             f'inside the trim-edge ringing zone')
                self.star_coords_pix = self.star_coords_pix[_keep]
            elif _keep.sum() == 0:
                self.sp_logger.warning(warn_y+' [V2] All detections sit in the trim-edge ringing zone — '
    ...
    def psf_fit(self,data_cutout,psf_array):
        ...
        for d in range(len(data_cutout)):
            ...
            if self.telescope in SEDM:
                ...
                if _clipped:
                    self.sp_logger.warning(warn_y + f' [V2] chi2_shift requested ({xoff_raw:+.1f},{yoff_raw:+.1f}) px '
    ...
    def get_zeropts(self):
        ...
        if len(self.matched_star_coords_pix)>0:
            ...
            if (hasattr(self, 'ref_sat_mask') and self.ref_sat_mask is not None
                ...
                if _n_drop > 0 and _n_keep >= 1:
                    ...
                    self.sp_logger.info(info_g + f' [V2] Dropped {_n_drop} zp calibrators whose reference '
                    ...
                    self.sp_logger.warning(warn_y + f' [V2] Saturation filter would reject all '
        ...
        if _n_drop_med > 0 and _keep_med.sum() >= 2:
            ...
            self.sp_logger.info(info_g + f' [V2] Median-zp filter: dropped {_n_drop_med} '
            ...
            self.sp_logger.warning(warn_y + f' [V2] Median-zp filter would leave <2 stars '
    ...
    def mag_err_function(self,data,psf,zp_sci,num,sn_x,sn_y):
        ...
        if _bkg_mad > 0 and _bkg_std > 2.5 * _bkg_mad:
            self.sp_logger.warning(warn_y+f' [v2] Background noise inflated by residuals: std={_bkg_std_original:.1f} vs MAD={_bkg_mad:.1f}')
        ...
        if _new_sn_mad > 0 and _new_sn_std_raw > 2.5 * _new_sn_mad:
            self.sp_logger.warning(warn_y + f' [v2] Artificial-SN scatter inflated by residuals: '
        ...
        self.sp_logger.info(info_g+' S/N (std artifical sn)= %.3f'%(sn_flux/max(_new_sn_std, 1e-30)))
        ...
        if _n_zp_min < 2:
            self.sp_logger.warning(warn_y + f' [V2] Only {_n_zp_min} calibrator stars survived '
    ...
    def scaled_subtract(self):
        ...
        if self.to_subtract==True:
            ...
            if (hasattr(self, 'ref_sat_mask') and self.ref_sat_mask is not None
                ...
                self.sp_logger.info(info_g + f' [V2] Blanked subtraction at '
    ...
    def get_photometry(self):
        ...
        self.sp_logger.info(info_g+f' Reference zeropoint std: {_ref_std_for_quad:.3f}'
        ...
        self.sp_logger.info(info_g+f' Science zeropoint std: {_sci_std_for_quad:.3f}'
