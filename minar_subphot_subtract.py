...
try:
    ...
    print(warn_r+f' Could not create entire_lc_imgs directory at {path}entire_lc_imgs: {e}')
...
if args.make_log!=False:
    ...
    if log_dest=='0':
        ...
        if out_dir=='by_obs_date':
            log_name = f"{path}{out_dir}/{DATE}_log.log"
        else:
                ...
                for file_entry in self.fits_dict[filt_key]:
                    try:
                        ...
                        if isinstance(file_entry[0], list):
                            fits_path = f"{self.path}{self.FOLDER}/{file_entry[0][0]}"
                        else:
                            fits_path = f"{self.path}{self.FOLDER}/{file_entry[0]}"
                ...
                for fit in sub_file:
                    ...
                    if not os.path.exists(dst):
                        try:
                            ...
                            sp_logger.warning(warn_r+f' Could not archive {fit} to {lc_path}: {e}')
...
if len(args.ims)>0:
    ...
    if args.stack==False:
        ...
        for i in range(len(ims)):
            ...
            sp_logger.info(colored('════════════════════════════════════════════════════════════════════════════════════════════════','magenta'))
            sp_logger.info(colored(f'  ⭐ PROCESSING FILE {i+1} OF {len(ims)} [{i+1}/{len(ims)}]', 'cyan'))
                                            ...
                                            if sys_exit==True:
                                                ...
                                                if sys_exit==True:
                                                    ...
                                                    if sys_exit==True:
                                                        ...
                                                        remaining = len(ims) - (i+1)
                                                        sp_logger.info(colored(f'✓ COMPLETED: {i+1}/{len(ims)} | {remaining} remaining', 'green'))
