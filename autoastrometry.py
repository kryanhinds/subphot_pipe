...
#
#    author: Daniel Perley (d.a.perley@ljmu.ac.uk)
#    last significant modifications 2019-01-29
#  
#  Installation:
...
try:
   import ephem
    ...
    print('Warning: ephem module not found.  Ephemeris-based coordinate transformations will not work.',e)
    pass
import warnings
...
def sextract(sexfilename, nxpix, nypix, border=3, corner=12, minfwhm=1.5, maxfwhm=25, maxellip=0.5, saturation=-1, quiet=False):
    ...
    # have to delete temp.cat; otherwise if it sextractor fails it will use an old version.
    if os.path.isfile('temp.cat'):
       os.system('rm -f temp.cat')

    try:
       # Sextract the image !
       os.system(sexpath+sexcmd+' ' + sexfilename + " -c sex.config -SATUR_LEVEL "+str(sexsaturation))
    except:
       print(' Error: Problem running sextractor')
       print(' Check that program is installed and runs at command line using ' + sexpath+sexcmd)
       sys.exit(1)
    ...
    try:
       ...
       print(' Error: Cannot load sextractor output file!')
       print(' Check that program is installed and runs at command line using ' + sexpath+sexcmd)
       sys.exit(1)
...
def read_downloaded_catalog(catlines, catalog):
    """
    Read catalog returned by WCSTools/SDSS after download.
    Handles SCAT-style tab tables and SDSS CSV.
    """

    # Decode bytes -> str
    lines = [l.decode("utf-8").rstrip() for l in catlines if l.strip()]
    ...
    # ----------------------------
    # SCAT catalogs (ub2, tmc, etc.)
    # ----------------------------
    # Find dashed separator line
    sep_idx = None
    ...
    if sep_idx is None:
        raise RuntimeError("Could not find SCAT table separator")

    # Table header is just above dashed line
...
def getcatalog(catalog, querycra, querycdec, boxsize, minmag=8.0, maxmag=-1, maxpm=60., tryc=False, quiet=False):
    ...
    cosdec = cos(querycdec*3.14159/180.)
    print(catalog, 'catalog: querying around RA=', querycra, 'DEC=', querycdec, 'boxsize=', boxsize, 'arcmin, mag<', maxmag, '...')
    sepchar = ''    
    if (catalog =='ub2' or catalog=='sdss' or catalog=='tmc'):
        ...
        if catalog=='ub2':
            ...
            pmdeccolumn = 12
        if catalog!='sdss':
           queryurl = "http://tdc-www.harvard.edu/cgi-bin/scat?catalog=" + catalog +  "&ra=" + str(querycra) + "&dec=" + str(querycdec) + "&system=J2000&rad=" + str(-boxsize) + "&sort=mag&epoch=2000.00000&nstar=6400"
        else:
           # The WCSTOOLS SDSS lookup doesn't function reliably anymore.
           # queryurl = 'http://skyserver.sdss.org/dr12/en/tools/search/x_radial.aspx?whichway=equitorial&ra='+str(querycra)+'&dec='+str(querycdec)+'&radius='+str(boxsize/60.)+'&min_u=0&max_u=30&min_g=0&max_g=30&min_r=0&max_r='+str(maxmag)+'&min_i=0&max_i=30&min_z=0&max_z=30&format=csv&limit=5000' #; DR12 pre-2016
           queryurl = 'http://skyserver.sdss.org/dr12/en/tools/search/x_results.aspx?searchtool=Radial&TaskName=Skyserver.Search.Radial&whichphotometry=optical&coordtype=equatorial&ra='+str(querycra)+'&dec='+str(querycdec)+'&radius='+str(boxsize/60.)+'&min_u=0&max_u=30&min_g=0&max_g=30&min_r=0&max_r=30&min_i=0&max_i=30&min_z=0&max_z=30&min_j=0&max_j=30&min_h=0&max_h=30&min_k=0&max_k=30&format=csv&limit=5000'
        # print(queryurl
        print(queryurl)
        try:
            ...
            print('Catalog query failed for', catalog, 'catalog:', e)
            return []
        # print(cat.readlines())
        # sys.exit()
        #if catalog =='sdss': 
        #  for line in catlines: print(line
        if tryc==True:
    ...
    #print('                ', maxmag, maxpm
    # print(catalog)
    # print('Reading catalog, length', len(catlines), '...')
    l = -1
    catlist = []
    fwhmlist = []
    # Normalize raw catalog lines into tokenized rows.
    parsed_rows = []
    if usercat:
        raw_rows = catlines
    elif sepchar == ',':
        ...
        raw_rows = catlines[14:]

    min_fields = max(racolumn, deccolumn, magcolumn, pmracolumn, pmdeccolumn) + 1
    for rawline in raw_rows:
        line = rawline.decode("utf-8") if isinstance(rawline, bytes) else str(rawline)
        line = line.strip()
        if len(line) == 0:
            continue
        if line.startswith('#') or line.startswith('----'):
            continue
        if line.lower().startswith('objid,'):
            continue

        inline = [tok.strip() for tok in line.split(',')] if sepchar == ',' else line.split()
        if len(inline) < min_fields:
            continue
        parsed_rows.append(inline)

    catlines = parsed_rows


    while l < len(catlines)-1:
        l += 1
        # inline = catlines[l].strip() #what does strip do?
        inline = catlines[l]
        #answer: 
        ...
        #      continue
        # racolumn = 
        # print(0<inline[0]<9, inline[0])
        # print(0<inline[1]<9, inline[1])
        # print(inline)
        if 'catalog' in str(inline).lower(): continue
        if 'usno-b1.0' in str(inline).lower(): continue

        try:
           ra = rasex2deg(inline[racolumn])
           dec = decsex2deg(inline[deccolumn])
           narg = len(inline)
           mag = float(inline[magcolumn])
           pmra = float(inline[pmracolumn])
           pmdec = float(inline[pmdeccolumn])
        except Exception:
           continue
        # if inlinearg[deccolumn].find(':') == -1:
        #    dec = float(inlinearg[deccolumn])
        # else:
        # #    if str(inlinearg).startswith('dec\t'):
        # #       d = inlinearg.split(b'\t', 1)[1].decode('utf-8')
        # #    else:
        #    d = inlinearg[deccolumn]
        #    dec = decsex2deg(d)
        ...
        if usercat:
           # for a usercat, have to do the boxsize filter manually (currently only circular search supported)
           centerdist = (((ra-querycra)*3600.*cosdec)**2 + ((dec-querycdec)*3600.)**2)**0.5
           if centerdist > abs(boxsize): continue
        #print(' OK',
    ...
    print(len(catlist), 'objects in catalog')
    # sys.exit()
    return catlist
...
def distmatch(sexlist, catlist, maxrad=180, minrad=10, tolerance=0.010, reqmatch=3, patolerance=1.2,uncpa=-1, quiet=False):
    ...
    print('All done (in', time.time()-dtime0, 's')
    ...
    print('All done (in', time.time()-mtime0, 's')
    ...
    if len(smatch) > 2:    
        print('Refined PA:', offpa, 'deg')
        print(patolerance, 'deg tolerance')
        #Coarse iteration for anything away from the mode
        for i in range(len(primarymatchs)-1,-1,-1):
            # print(i, mpa[i], offpa, abs(mpa[i] - offpa), patolerance)
            if abs(mpa[i] - offpa) > patolerance*2.5:
                del mpa[i]
                del primarymatchs[i]
                del primarymatchc[i]
                del smatch[i]
        ...
        print('Refined PA:', medpa, 'deg')
        print(refinedtolerance, 'deg tolerance')
        #Fine iteration to flag outliers now that we know most are reliable
        for i in range(len(primarymatchs)-1,-1,-1): 
            print(i, mpa[i], offpa, abs(mpa[i] - offpa), refinedtolerance)
            if abs(mpa[i] - offpa) > refinedtolerance*2.5:
                del mpa[i]
                del primarymatchs[i]
                del primarymatchc[i]
                del smatch[i]
...
def autoastrometry(filename,pixelscale=-1,pa=-999,inv=0,uncpa=-1,userra=-999, userdec=-999,
catcenterra=-999, catcenterdec=-999,
minfwhm=1.5,maxfwhm=20,maxellip=0.5,boxsize=-1,maxrad=-1,tolerance=0.010,catalog='',nosolve=0,
overwriteself=False, outfile='', saturation=-1, quiet=False, norot=0):
    ...
    if pixelscale < 0:
       try:
          ...
          hkeys = []
          for card in h.cards: hkeys.append(card.keyword)
       oldwcstype = 0
       for hkey in hkeys:
       ...
       if oldwcstype:
          ...
          if sqrt(cdelt1**2 + cdelt2**2) < 0.1:  # some images use CDELT to indicate nonstandard things
             h.set("CD1_1", cdelt1 * cos(crot*math.pi/180.))
             h.set("CD1_2",-cdelt2 * sin(crot*math.pi/180.))
             h.set("CD2_1", cdelt1 * sin(crot*math.pi/180.))
             h.set("CD2_2", cdelt2 * cos(crot*math.pi/180.))
    ...
    if pixelscale > 0 and pa > -360:
       ...
       if userra >= 0. and userra < 360.:
          ...
          try:
             ...
             try:
                ...
                print(' No positional information (RA/DEC or CRVAL1/CRVAL2) in header.')
                print('    Specify at command line:  -ra [val] -dec [val].')
                return -1
       ...
       if abs(equinox-2000) > 0.5:
           ...
           try:
              ...
              return -1
       h.set("CD1_1",  pxscaledeg * cos(parad)*parity)
       h.set("CD1_2",  pxscaledeg * sin(parad))
       h.set("CD2_1", -pxscaledeg * sin(parad)*parity)
       h.set("CD2_2",  pxscaledeg * cos(parad))
       ...
       #h.set("EPOCH",2000.0)  does it matter?
    ...
    try:
        ...
        cd22 = float(h[key])
        key = 'CD1_2'
        cd12 = h.get(key,0.) # allowing this to be absent, since I have 
        key = 'CD2_1'        # encountered files with only CDi_i terms...
        cd21 = h.get(key,0.)
        ...
        if pixelscale == -1:
            print('Cannot find necessary WCS header keyword', key)
            #calculate pxelscale=sqrt(CD1_1*2 + CD1_2*2)
    ...
    distortionkeys = []
    for hkey in hkeys:
       if hkey=='RADECSYS' or hkey=='RADESYS' or hkey == 'WCSDIM' or hkey.find('WAT')==0: # or hkey.find('LTV')>=0 or hkey.find('LTM')==0:
          del h[hkey]
          irafkeys.append(hkey)
       if hkey.find('CO1_')==0 or hkey.find('CO2_')==0 or hkey.find('PC00')==0: # or hkey.find('PV1_')==0 or hkey.find('PV2_')==0
          del h[hkey]
          highkeys.append(hkey)
       if hkey.find('CDELT1')==0 or hkey.find('CDELT2')==0 or hkey.find('CROTA1')==0 or hkey.find('CROTA2')==0:
          del h[hkey]
          oldkeys.append(hkey)
       #if hkey.find('AP_')==0 or hkey.find('BP_')==0 or hkey.find('A_')==0 or hkey.find('B_')==0 or 
       #   del h[hkey]
       #   distortionkeys.append(hkey)
    if h['CTYPE1'] != 'RA---TAN' and h['CTYPE1'] != 'RA---TAN-SIP': 
       if quiet==False: print('Changing CTYPE1 from',h['CTYPE1'],'to',"RA---TAN")
       h.set("CTYPE1","RA---TAN")
       ctypechange = 1
    if h['CTYPE2'] != 'DEC--TAN' and h['CTYPE2'] != 'DEC--TAN-SIP': 
       if quiet==False: print('Changing CTYPE2 from',h['CTYPE2'],'to',"DEC--TAN")
       h.set("CTYPE2","DEC--TAN")
       ctypechange = 1
    ...
    if quiet == False:
       ...
       #   print('Removed distortion WCS keywords: ',
       #   for key in distortionkeys: print(key,
       #   print(   
    ...
    # reversed the sign on + centerdx for images with negative cdelt.  rare scenario?
  
    if catcenterra==-999: catcenterra = centerra
    if catcenterdec==-999: catcenterdec = centerdec
    if quiet == False:
    ...
    # Tune defaults using instrument/header context when explicit CLI values were not provided.
    inst = str(h.get('INSTRUME', '')).upper()
    telescop = str(h.get('TELESCOP', '')).upper()
    is_sedm = ('SEDM' in inst) or ('P60' in telescop) or os.path.basename(filename).startswith('rc')

    if saturation <= 0:
        for skey in ('SATURATE', 'SATURAT', 'SATLEVEL', 'SATUR_LEVEL'):
            if skey in h:
                try:
                    saturation = float(h[skey])
                    break
                except Exception:
                    pass
        if saturation <= 0 and is_sedm:
            # Typical P60/SEDM rcam saturation is around this level in ADU.
            saturation = 50000.0

    if minfwhm == defaultminfwhm and maxfwhm == defaultmaxfwhm:
        header_seeing_pix = -1.0
        for pkey in ('SEEINGPIX', 'FWHMPIX', 'L1SEEPX'):
            if pkey in h:
                try:
                    header_seeing_pix = float(h[pkey])
                    if header_seeing_pix > 0:
                        break
                except Exception:
                    pass

        if header_seeing_pix <= 0:
            for skey in ('L1SEESEC', 'SEEING', 'FWHM', 'ESO TEL AMBI FWHM END'):
                if skey in h:
                    try:
                        see_arcsec = float(h[skey])
                        if see_arcsec > 0 and pixscale > 0:
                            header_seeing_pix = see_arcsec / pixscale
                            break
                    except Exception:
                        pass

        if header_seeing_pix > 0:
            # Use tighter star-selection bounds than the legacy broad defaults.
            minfwhm = max(0.9, 0.35 * header_seeing_pix)
            maxfwhm = min(15.0, max(4.0, 2.0 * header_seeing_pix))
        elif is_sedm:
            # Conservative fallback for SEDM when no seeing header is present.
            minfwhm = 1.2
            maxfwhm = 8.0
    ...
    goodsexlist = sextract(sfilename, nxpix, nypix, 3, 12, minfwhm=minfwhm, maxfwhm=maxfwhm, maxellip=maxellip, saturation=saturation, quiet=quiet)
    ...
    if (boxsize == -1):
        boxsize = fieldwidth
        if is_sedm:
            # Keep a tighter query region for SEDM to reduce false/slow catalog matches.
            boxsize = min(boxsize, 300.0)

    # Load in reference star catalog
    if catalog != '':
        catlist = getcatalog(catalog, catcenterra, catcenterdec, boxsize, tryc=False, quiet=quiet)
        # sometimes the catalog covers a smaller area than requested, so correct catalog area measurement (for density estimate)
        minra = 360. # note that this will not deal with meridian-crossing fields properly.
        mindec = 90.
        maxra = 0.
        maxdec = -90.
        for c in catlist:
           if c.ra < minra: minra = c.ra
           if c.dec < mindec: mindec = c.dec
           if c.ra > maxra: maxra = c.ra
           if c.dec > maxdec: maxdec = c.dec
        cosdec = cos(centerdec*3.14159/180.)
        catboxeffdiam = (((maxra-minra)*cosdec*3600.)**2 + ((maxdec-mindec)*3600.)**2)**0.5
        if catboxeffdiam > boxsize: catboxeffdiam = 2*boxsize # if catalog isn't obviously a smaller field, revert to boxsize
    # If no catalog specified, try SDSS, and if nothing returns try USNO
        ...
        catboxeffdiam = 2*boxsize
        for trycat in trycats:
            catlist = getcatalog(trycat, catcenterra, catcenterdec, boxsize, tryc=True, quiet=quiet)
            if len(catlist) >= 3:
                mindist = 999999.
                cosdec = cos(centerdec*3.14159/180.)
                for c in catlist:
                   dist = abs(c.ra - centerra)*cosdec/3600. + abs(c.dec - centerdec)/3600.
                   if dist < mindist: mindist = dist
                if mindist > 90.: continue  # sometimes just off an edge of SDSS
                if quiet==False: print('Using catalog', trycat)
                catalog = trycat
                break
        if (catalog == ''):
    ...
    ncat = len(catlist)
    catdensity = ncat / (catboxeffdiam/60.)**2
    if quiet==False: 
    ...
    (primarymatchs, primarymatchc, mpa) = distmatch(goodsexlist, catlist, maxrad, minrad, tolerance, reqmatch, patolerance, uncpa, quiet=False)
    ...
    if norot <= 0:
       ...
       rot = offpa * math.pi/180
         #...the image itself
       h.set("CD1_1", cos(rot)*cd11 - sin(rot)*cd21 )
       h.set("CD1_2", cos(rot)*cd12 - sin(rot)*cd22 )  # a parity issue may be involved here?
       h.set("CD2_1", sin(rot)*cd11 + cos(rot)*cd21 )
       h.set("CD2_2", sin(rot)*cd12 + cos(rot)*cd22 )
    ...
    if (stdoffset*3600 > 3.0):
        print("Extremely poor (wrong) solution.")
        fits.close()
        return -1

    if quiet==True:
       print('  Succeeded with%3i matches.  dPA = %6.3f (unc %.3f) dra =%7.2f" ddec =%7.2f" (unc %.3f")' % (nmatch, skyoffpa, stdevpa, raoffsetarcsec, decoffsetarcsec, stdoffsetarcsec))
    ...
    #h.set("ASTRMTCH", catalog)
    try:
       oldcat = h['ASTR_CAT']
       h.set("OLD_CAT",oldcat, "Earlier reference catalog")
    except:
       pass
    h.set("ASTR_CAT", catalog, "Reference catalog for autoastrometry")
    ...
    if overwriteself: outfile = filename
    if outfile == '': 
...
#          12345678901234567890123456789012345678901234567890123456789012345678901234567890
    ...
    print("      -k:              Do not overwrite output file if it exists")
...
def main():
    ...
    userra = -999
    userdec = -999
    catcenterra = -999 # used to force the catalog to query only around a region that is not necessarily the image center.
    catcenterdec = -999
    ...
    while (i<len(sys.argv)):
        ...
        if (arg.find("-dec") == 0):
            ...
            isarg=1
        if (arg.find("-cra") == 0):
            catcenterra = rasex2deg(sys.argv[i+1])
            i+=1
            isarg=1
        if (arg.find("-cdec") == 0):
            catcenterdec = decsex2deg(sys.argv[i+1])
            i+=1
            isarg=1
        if (arg.find("-pa") == 0):
        ...
        if (arg.find("-o") == 0):
            ...
            if len(sys.argv) > i+1:
               if (sys.argv[i+1]).find('-') != 0: 
                  outfile = sys.argv[i+1] #output, currently can only be a scalar
                  i+=1
            if outfile == '': overwriteself = 1
            isarg = 1
    ...
    if overwriteself and noclobber:
        print('Cannot simultaneously have -o (self-overwrite) and -k (never overwrite)')
        return
    ...
    if len(filenames)>1 and len(outfile)==1 and outfile != '':
        print('Cannot specify multiple input filenames and only one output file')
        return
    ...
    for filename in filenames:
        ...
        if outfile != '':
           ...
           if overwriteself==True: 
              outfilef=filename
           else:
              slashpos = filename.rfind('/')
              dir = filename[0:slashpos+1]
              fil = filename[slashpos+1:]
              outfilef = dir+'a'+fil # alternate behavior would always output to current directory
        if noclobber==True and os.path.exists(outfilef):
           print(outfilef, 'exists; skipping.')
           skipped.append(filename)
           multiinfo.append((-2,0,0,0,0,0))
           continue

        if len(filenames) > 1: print('Processing', filename)
        if nosolve and catalog=='': catalog = filename+'.cat'
        fitinfo = autoastrometry(filename,pixelscale=pixelscale,pa=pa,inv=inv,uncpa=uncpa,minfwhm=minfwhm,maxfwhm=maxfwhm,maxellip=maxellip,boxsize=boxsize, maxrad=maxrad, userra=userra, userdec=userdec, catcenterra=catcenterra, catcenterdec=catcenterdec, tolerance=tolerance, catalog=catalog, nosolve=nosolve, overwriteself=overwriteself, outfile=outfilef, saturation=saturation, quiet=quiet, norot=norot)
        if fitinfo==-1:
            print('  Skipping', filename)
            skipped.append(filename)
        if nosolve: continue
        if type(fitinfo)==int:  # failure
           fitinfo = (0,0,0,0,0,0)  
           nfailed += 1
        else:
           nsolved += 1
    ...
    if nimage > 1 and nsolved+nfailed>0 and nosolve==0:
        ...
        for i in range(len(filenames)):
            info = multiinfo[i]
            if info[0] == -2: continue # don't print(anything if skipped
            print("%25s " % filenames[i], )
            if info[0] > 0:
               print("%6d %8.3f (%6.3f)  %7.3f %7.3f (%6.3f)" % info)
            else:
               if info[0] == 0: print("failed to solve")
...
# program can actually be overwhelmed by too many good matches (too high maxrad).
