...
def getcatalog(catalog, querycra, querycdec, boxsize, minmag=8.0, maxmag=-1, maxpm=60., tryc=False, quiet=False):
   ...
   #  print(catlines)
    while l < len(catlines)-1:
        ...
        if b'#Table1' == inline: continue
        if b'objid,run,rerun,camcol,field,obj,type,ra,dec,u,g,r,i,z,Err_u,Err_g,Err_r,Err_i,Err_z' == inline: continue
        if any(x in inline for x in [b'catalog',b'USNO-B1.0']): continue
      #   print(inline)
        ...
        if sepchar == '': inlinearg = inline.decode("utf-8").split()
        if sepchar != '': inlinearg = inline.decode("utf-8").split(sepchar)
        narg = len(inlinearg)
        # print(mag, ra, dec, pmra, pmdec,l)
...
overwriteself=False, outfile='', saturation=-1, quiet=False, norot=0):
    ...
    (primarymatchs, primarymatchc, mpa) = distmatch(goodsexlist, catlist, maxrad, minrad, tolerance, reqmatch, patolerance, uncpa, quiet=quiet)
